...
import me.bedtrapteam.addon.util.autocrystalrewrite.BlockPoz;
import me.bedtrapteam.addon.util.autocrystalrewrite.TimerUtils;
...
import static me.bedtrapteam.addon.util.feettrap.FTUtils.*;
...
public class FeetTrap extends Module {
    ...
    private final Setting<SettingColor> sideColor = sgRender.add(new ColorSetting.Builder().name("side-color").defaultValue(new SettingColor(255, 0, 170, 10)).visible(() -> visibleSide(shapeMode.get())).build());
    private final Setting<SettingColor> lineColor = sgRender.add(new ColorSetting.Builder().name("line-color").defaultValue(new SettingColor(255, 0, 170, 90)).visible(() -> visibleLine(shapeMode.get())).build());
    private final Setting<Double> height = sgRender.add(new DoubleSetting.Builder().name("height").defaultValue(0.99).sliderRange(0, 1).visible(() -> visibleHeight(renderMode.get())).build());
    ...
    public FeetTrap() {
        super(BedTrap.Combat, "QQuiver", "Automatically places blocks around your feet.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        ArrayList<BlockPoz> boobies = getSurroundBlocks(mc.player);
    ...
    public ArrayList<BlockPoz> getPositions(PlayerEntity player) {
        ...
        for (BlockPoz blockPos : getSphere(new BlockPoz(player.getBlockPos()), 3, 1)) {
    ...
    public void onRender(Render3DEvent event) {
        if (renderMode.get() == RenderMode.None || poses.isEmpty()) return;
        RenderInfo ri = new RenderInfo(event, renderMode.get(), shapeMode.get());

        poses.forEach(blockPos -> render(ri, blockPos, sideColor.get(), lineColor.get(), height.get()));
    }
}
