package me.bedtrapteam.addon.util.autocrystalrewrite;
...
public class ACUtils {
    public static List<BlockPoz> getSphere(PlayerEntity player, double radius) {
        ...
        for (double i = eyePos.getX() - radius; i < eyePos.getX() + radius; i++) {
            for (double j = eyePos.getY() - radius; j < eyePos.getY() + radius; j++) {
                for (double k = eyePos.getZ() - radius; k < eyePos.getZ() + radius; k++) {
                    ...
                    BlockPoz blockPos = new BlockPoz(vecPos);
    ...
    public static EndCrystalEntity getEntity(BlockPoz blockPos) {
    ...
    public  static Vec3d roundVec(Entity entity) {
        BlockPoz blockPos = new BlockPoz(entity.getBlockPos().down());

        return new Vec3d(blockPos.getX() + 0.5, blockPos.getY() + 1, blockPos.getZ() + 0.5);
    }

    public static Vec3d roundVec(BlockPoz blockPos) {
        return new Vec3d(blockPos.getX() + 0.5, blockPos.getY() + 1, blockPos.getZ() + 0.5);
    }

    public static double distanceTo(BlockPoz blockPos) {
    ...
    public static List<BlockPoz> getSurroundBlocks(PlayerEntity player, boolean allBlocks) {
        ...
        for (BlockPoz blockPos : getSphere(new BlockPoz(player.getBlockPos()), 3, 1)) {
            ...
            for (Direction direction : Direction.values()) {
                ...
                if (!getEntityBoxes.isEmpty()) positions.add(new BlockPoz(blockPos));
            }
        }
    ...
    public static boolean isFaceTrapped() {
        ...
        for (BlockPoz blockPos : getSphere(new BlockPoz(player.getBlockPos().up(2)), 3, 1)) {
    ...
    public static List<BlockPoz> getSphere(BlockPoz centerPos, int radius, int height) {
        ArrayList<BlockPoz> blocks = new ArrayList<>();
